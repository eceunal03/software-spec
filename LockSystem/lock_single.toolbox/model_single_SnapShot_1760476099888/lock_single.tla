      ...
      if ~InLock then
        ...
        PrepareLock:
          if (lockOrientation = "west_low" /\ req.side = "west")
            ...
            \* --- Close opposite door first ---
            CloseOtherDoorLow:
              if req.side = "west" then
            ...
            \* --- Adjust valves for low water level ---
            SetLowValve:
            ...
            \* --- Adjust valves for high water level ---
            SetHighValve:
...
\* BEGIN TRANSLATION (chksum(pcal) = "db8e2adf" /\ chksum(tla) = "8f49059d")
...
ShipMoveWest(self) == /\ pc[self] = "ShipMoveWest"
                      /\ IF perm[self].granted
                            THEN /\ Assert(doorsOpen[IF InLock THEN "west" ELSE "east"], 
                                           "Failure of assertion at line 214, column 13.")
                                 /\ shipLocation' = shipLocation - 1
                            ELSE /\ TRUE
                                 /\ UNCHANGED shipLocation
                      /\ pc' = [pc EXCEPT ![self] = "ShipNextIteration"]
...
PrepareLock == /\ pc[0] = "PrepareLock"
               /\ IF (lockOrientation = "west_low" /\ req.side = "west")
                     ...
                     THEN /\ pc' = [pc EXCEPT ![0] = "CloseOtherDoorLow"]
                     ELSE /\ pc' = [pc EXCEPT ![0] = "CloseOtherDoorHigh"]
               /\ UNCHANGED << lockOrientation, doorsOpen, valvesOpen, 
                               waterLevel, shipLocation, shipStatus, 
                               lockCommand, requests, permissions, perm, req >>

CloseOtherDoorLow == /\ pc[0] = "CloseOtherDoorLow"
                     /\ IF req.side = "west"
                           THEN /\ lockCommand' = [command |-> "change_door", open |-> FALSE, side |-> "east"]
                                /\ lockCommand'.command = "finished"
                           ELSE /\ lockCommand' = [command |-> "change_door", open |-> FALSE, side |-> "west"]
                                /\ lockCommand'.command = "finished"
                     /\ pc' = [pc EXCEPT ![0] = "SetLowValve"]
                     /\ UNCHANGED << lockOrientation, doorsOpen, valvesOpen, 
                                     waterLevel, shipLocation, shipStatus, 
                                     requests, permissions, perm, req >>
...
CloseOtherDoorHigh == /\ pc[0] = "CloseOtherDoorHigh"
                      /\ IF req.side = "west"
                            THEN /\ lockCommand' = [command |-> "change_door", open |-> FALSE, side |-> "east"]
                                 /\ lockCommand'.command = "finished"
                            ELSE /\ lockCommand' = [command |-> "change_door", open |-> FALSE, side |-> "west"]
                                 /\ lockCommand'.command = "finished"
                      /\ pc' = [pc EXCEPT ![0] = "SetHighValve"]
                      /\ UNCHANGED << lockOrientation, doorsOpen, valvesOpen, 
                                      waterLevel, shipLocation, shipStatus, 
                                      requests, permissions, perm, req >>
...
controlProcess == ControlLoop \/ WaitForRequest \/ CheckSafety
                     \/ PrepareLock \/ CloseOtherDoorLow \/ SetLowValve
                     \/ OpenLowDoor \/ CloseOtherDoorHigh \/ SetHighValve
                     \/ OpenHighDoor \/ GrantPermission \/ CloseDoor
                     \/ DenyRequest
...
\* Last modified Tue Oct 14 23:08:12 CEST 2025 by 20241799
